...
- **Agents report their own progress.** `herdr-projects report --percent N --activity "..."`, run by the agent in its pane, writes one small JSON file per pane under `<root>/.progress/` which the ticker shows on the pane's rail for five minutes. Hooks in Claude Code and Codex (installed by `configure`) inject the instructions and a reminder. There is no daemon and no database.
