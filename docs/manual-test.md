...
| 3 | `thread start --model` records the model and the ticker launches with `--model <id>` after `thread_agent_args`; without the flag `thread_model` from `PROJECT.md` applies; `thread restart` keeps it; an empty id or one starting with a dash is refused | Unit. Not yet run against a real agent |
