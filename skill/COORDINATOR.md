...
- `--agent <kind>` picks the harness for this thread (any Herdr agent kind: `claude`, `codex`, `omp`, `opencode`, `cursor`, `gemini`, ...). Without an explicit selection, the user's default thread profile takes precedence over `thread_agent` in the settings. Explicit `--agent` opts out of that profile; use it only when an unprofiled launch is intended.
...
- `--profile <name>` selects an administrator-defined full launch configuration; `context` and `hp safety show <slug>` list the allowed names for each role. Use distinct trusted profiles for independent workers (for example Astra and Sol) rather than passing OMP `--config` through `--agent-arg` or reducing the configuration to a model flag. Profiles replace the role's legacy launch arguments and cannot be combined with `--agent` or model overrides. If the needed name is not permitted, ask the user to define/authorize it; never edit the registry or allow-lists yourself.
- `hp thread restart <slug> <id>` retains its selected profile. `--profile <name>` switches to another permitted profile; `--agent <kind>` removes the previous profile. The ticker checks authorization again before launch; a revoked or missing profile fails rather than silently starting an unprofiled agent.
