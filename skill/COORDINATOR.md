...
- `--agent-arg <arg>` (repeatable) passes extra arguments to that harness, which is how you apply a model. Model flags per harness: Claude Code `--agent-arg --model --agent-arg <name>`; Codex `--agent-arg --model --agent-arg <name>` (also `-m`); Gemini CLI `--agent-arg --model --agent-arg <name>`; OpenCode `--agent-arg --model --agent-arg <provider/model>`; Cursor Agent `--agent-arg --model --agent-arg <name>`; Copilot CLI `--agent-arg --model --agent-arg <name>`. For another harness, ask the user for its flag once and remember it. A running thread switches model with its harness's own `/model`; to switch harness, restart it: `hp thread restart <slug> <id> --agent <kind>`.
